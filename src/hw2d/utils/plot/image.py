...
def main(
    ...
    t: float = 200,
    save_types: Iterable[str] = (".jpg", ".pdf"),
    ...
    with h5py.File(file_path, "r") as file:
        ...
        t_idx = int((t - parameters.get("initial_time", 0)) // parameters["frame_dt"])
        print(f"{t} for file starting at {parameters.get('initial_time', 0)} is {t_idx:,}")
    ...
    if save_file_path:
        for save_type in save_types:
            fig.savefig(save_file_path[:-4] + save_type)
